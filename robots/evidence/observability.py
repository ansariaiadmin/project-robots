...
class ObservabilityHooks:
    ...
    def to_dict(self) -> dict:
        return {
            "metrics": [
                {"name": m.name, "type": m.type, "description": m.description, "labels": m.labels, "unit": m.unit}
                for m in self.metrics
            ],
            "traces": [{"name": t.name, "attributes": t.attributes, "parent": t.parent} for t in self.traces],
            "logs": [{"level": log.level, "message": log.message, "fields": log.fields} for log in self.logs],
            "alerts": self.alerts,
    ...
    def to_otel_instrumentation(self) -> str:
        ...
        nl = "\n"
        metrics_code = nl.join(
            f'{m.name} = meter.create_{m.type}("{m.name}", description="{m.description}", unit="{m.unit}")'
            for m in self.metrics
        )
        traces_code = nl.join(
            f'with tracer.start_as_current_span("{t.name}") as span:\n    span.set_attributes({t.attributes})'
            for t in self.traces
        )
        logs_code = nl.join(f'logger.{log.level}("{log.message}", extra={log.fields})' for log in self.logs)
