...
class ContractDiffAnalyzer:
    ...
    def _diff_contracts(self, old: dict, new: dict) -> dict | None:
        ...
        if old.get("type") in {"pydantic", "dataclass", "typeddict"} and new.get("type") in {"pydantic", "dataclass", "typeddict"}:
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> None:
        ...
        for item in node.body:
            if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                if item.annotation:
                    field_types[item.target.id] = ast.unparse(item.annotation) if hasattr(ast, 'unparse') else str(item.annotation)
