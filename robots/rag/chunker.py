...
def chunk_python_symbols(
    ...
    for node in tree.body:
        if isinstance(node, (ast.Import, ast.ImportFrom)) or (isinstance(node, ast.Expr) and header_end == 0):
            ...
            if methods and span > max_lines:
                ...
                for method in methods:
                    ...
                    if m_end - m_start + 1 > max_lines:
                        chunks.extend(_split_oversized(path, f"{node.name}.{method.name}", m_start, lines))
                    else:
                        chunks.append(
                            {
                                "path": path,
