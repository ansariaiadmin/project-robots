...
class Critic:
    ...
    def _check_abstraction_level(
        self, plan: dict, intelligence: RepositoryIntelligence, issue: str
                            ...
                            if not boundary.allowed:
                                findings.append(
                                    CritiqueFinding(
                                        ...
                                        file=file_path,
                                        line=edge.line,
                                        suggestion="Move dependency to appropriate layer or use dependency inversion",
                                    )
                                )

        return findings

    def _check_error_surface(
        self, plan: dict, intelligence: RepositoryIntelligence, issue: str
    ) -> list[CritiqueFinding]:
        ...
        for file_path in changed_files:
            if file_path in intelligence.import_graph:
                ...
                if node.imported_by:  # Has dependents
                    ...
                    has_migration = any("migrate" in str(s).lower() for s in plan.get("steps", []))
