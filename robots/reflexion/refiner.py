...
class Refiner:
    ...
    def _refine_abstraction_level(
        self, steps: list[dict], findings: list, intelligence: RepositoryIntelligence
    ) -> list[dict]:
    ...
    def _refine_adr_quality(self, plan: dict, findings: list) -> dict:
        ...
        for finding in findings:
            if "missing" in finding.message.lower():
                ...
                # TASK #5: Close TODO — do not ship TODO placeholder as evidence
                # Use actual suggestion content instead of TODO prefix
                suggestion = getattr(finding, "suggestion", "") or finding.message
                refined["adr"][field] = suggestion.strip() if isinstance(suggestion, str) else str(suggestion)
